...
pub struct MintMakerRunner {
    ...
    /// Market IDs we've already placed on (never place twice on same market)
    placed_markets: Mutex<HashSet<String>>,
...
impl MintMakerRunner {
    pub fn new(
                ...
                match price_rx.recv().await {
                    Ok(update) => {
                        if let Ok(price) = Decimal::from_str(&update.price) {
                            cache.write().await.insert(update.token_id, price);
                        }
                    }
        ...
        Self {
            ...
            placed_markets: Mutex::new(HashSet::new()),
            price_cache,
            mm_live_tokens,
    ...
    async fn run_cycle(
        ...
        // Overlay real-time CLOB prices from the price WebSocket onto live markets.
        // A market is "live" if it closes within 15 minutes (i.e. currently in its trading window).
        {
            let cache = self.price_cache.read().await;
            let mut live_tokens = HashSet::new();
            for market in &mut all_markets {
                if market.minutes_to_close <= 15.0 && market.minutes_to_close > 0.0 {
                    // This market is in its active window — register tokens for price WS
        ...
        // Process each enabled wallet (uses per-wallet time filter for auto-placement)
        for wallet_address in &enabled_wallets {
            if let Err(e) = self
    ...
    async fn process_wallet(
        ...
        // 1b. Stop loss on half-filled pairs: if the filled side dropped 25%,
        //     cancel the unfilled order and sell the filled shares to cut losses.
        let half_filled_pairs = open_pairs.iter().filter(|p| p.status == "HalfFilled");
        for pair in half_filled_pairs {
        ...
        // 1c. Track stop loss sell orders — check if the sell filled
        let stop_loss_pairs = self.db.get_mint_maker_stop_loss_pairs(wallet_address).await?;
        for pair in &stop_loss_pairs {
            if let Some(ref sl_order_id) = pair.stop_loss_order_id {
                self.check_stop_loss_fill(
                    wallet_address,
                    pair,
                    sl_order_id,
                    &api_key,
        ...
        // 3. Cancel stale pairs
        let stale_pairs = self
            .db
            .get_mint_maker_stale_pairs(wallet_address, settings.stale_order_seconds)
            .await?;
        ...
        if settings.auto_place {
            ...
            if let Some(private_key) = self.key_store.get_key(wallet_address).await {
                ...
                // USD per side (auto_place_size is now in USD, not shares)
                let usd_per_side =
                    ...
                    Ok(safe_addr) => {
                        order_manager::fetch_safe_usdc_balance(&self.client, &safe_addr)
                            ...
                            .unwrap_or_else(|e| {
                                ...
                                Decimal::from(9999) // assume sufficient if check fails
                            })
                    }
                    Err(_) => Decimal::from(9999),
                };
                let mut remaining_balance = safe_balance;
                ...
                // Auto-place on markets the INSTANT they enter their 15-min trading window.
                // "Live" = minutes_to_close <= 15 and > 0.
                // On first cycle, seed placed_markets with all currently live markets (don't buy mid-market).
                // After that, any live market not in placed_markets = just opened = BUY NOW.
                let mut placed = self.placed_markets.lock().await;

                let live_markets: Vec<&MintMakerMarket> = eligible_markets.iter()
                    .filter(|m| m.minutes_to_close <= 15.0 && m.minutes_to_close > 0.0)
                    .collect();

                // First cycle: seed all currently live markets as "already placed"
                if placed.is_empty() && !live_markets.is_empty() {
                    for m in &live_markets {
                        placed.insert(m.market_id.clone());
                    }
                    info!(
                        "MintMaker: First cycle — seeded {} live markets as already placed (waiting for next open)",
                        live_markets.len()
                    );
                    drop(placed);
                } else {
                    // Find live markets we haven't placed on yet = JUST OPENED
                    let new_markets: Vec<&MintMakerMarket> = live_markets.iter()
                        .filter(|m| !placed.contains(&m.market_id))
                        .copied()
                        .collect();

                    // Clean up: remove market IDs no longer in eligible set
                    let current_ids: HashSet<String> = eligible_markets.iter().map(|m| m.market_id.clone()).collect();
                    placed.retain(|id| current_ids.contains(id));

                    if new_markets.is_empty() {
                        info!(
                            "MintMaker: Waiting — {} eligible, {} live, {} placed",
                            eligible_markets.len(), live_markets.len(), placed.len()
                        );
                    } else {
                        let bid_off = Decimal::from(settings.bid_offset_cents) / Decimal::from(100);
                        info!("MintMaker: {} market(s) JUST OPENED! {:?}",
                            new_markets.len(),
                            new_markets.iter()
                                .map(|m| format!("{} {:.1}m mid YES@{} NO@{} → bid YES@{} NO@{} cost={}",
                                    m.asset, m.minutes_to_close, m.yes_price, m.no_price,
                                    m.yes_price - bid_off, m.no_price - bid_off,
                                    (m.yes_price - bid_off) + (m.no_price - bid_off)))
                                .collect::<Vec<_>>()
                        );
                    }
                ...
                let max_cost = Decimal::from_str(&format!("{:.4}", settings.max_pair_cost)).unwrap_or(Decimal::from_str("0.98").unwrap());
                let min_profit = Decimal::from_str(&format!("{:.4}", settings.min_spread_profit)).unwrap_or(Decimal::from_str("0.01").unwrap());

                for market in &new_markets {
                    // Respect auto_max_markets setting
                    ...
                    if total_open >= settings.max_total_pairs as i64 {
                        info!("MintMaker: Hit max_total_pairs limit ({})", settings.max_total_pairs);
                        break;
                    }

                    // Check per-market capacity
                    ...
                    if market_pairs >= settings.max_pairs_per_market as i64 {
                        info!("MintMaker: Skipping {} — already at max pairs for this market", market.asset);
                        continue;
                    }

                    // Apply bid offset (same as calculate_bids) — bid below midpoint
                    let offset = Decimal::from(settings.bid_offset_cents) / Decimal::from(100);
                    let yes_price = market.yes_price - offset;
                    let no_price = market.no_price - offset;

                    // Validate bids are positive
                    if yes_price <= Decimal::ZERO || no_price <= Decimal::ZERO {
                        info!("MintMaker: SKIP {} — bid price would be <= 0 after offset", market.asset);
                        continue;
                    }

                    // Profitability check at bid prices (with offset applied)
                    let actual_pair_cost = yes_price + no_price;
                    if actual_pair_cost > max_cost {
                        info!(
                            "MintMaker: SKIP {} — pair cost {} > max {} (bid YES@{} NO@{})",
                            market.asset, actual_pair_cost, max_cost, yes_price, no_price
                        );
                        continue;
                    }
                    ...
                    // Count this market attempt toward the limit (even if it fails)
                    markets_placed += 1;

                    // Place YES as GTC at scanner price (aggressive limit for fast fill)
                    let yes_order_id = match order_manager::place_gtc_bid(
                        &private_key,
                        &market.yes_token_id,
                        yes_price,
                        yes_shares,
                    ...
                    .await
                    {
                        Ok(id) => id,
                        Err(e) => {
                            warn!("MintMaker auto-place YES GTC failed for {}: {}", market.market_id, e);
                            continue;
                        }
                    };
                    ...
                    let no_order_id = match order_manager::place_gtc_bid(
                        &private_key,
                        &market.no_token_id,
                        no_price,
                        no_shares,
                    ...
                    .await
                    {
                        ...
                        Err(e) => {
                            warn!(
                                "MintMaker auto-place NO GTC failed, cancelling YES {}: {}",
                                yes_order_id, e
                            );
                    ...
                    // These are aggressive limits at scanner price — should fill quickly
                    // on fresh markets, and qualify as maker orders (0% fee) if they rest.
                    ...
                    // Mark this market as placed so we don't place again
                    self.placed_markets.lock().await.insert(market.market_id.clone());
                }
                } // end else (not first cycle)
            } else {
                warn!(
    ...
    async fn check_stop_loss(
        ...
        // Don't stop-loss too quickly — the other side may still fill.
        // Wait at least 5 minutes from pair creation before considering stop loss.
        if let Ok(created) = chrono::DateTime::parse_from_rfc3339(&pair.created_at) {
        ...
        // Find the matching market in scanner data to get current price + token ID
        let market = match raw_markets.iter().find(|m| m.condition_id == pair.condition_id) {
            Some(m) => m,
            None => return, // market no longer in scanner (may have closed)
        };

        // Current price of the filled side
        let current_price = if yes_filled { market.yes_price } else { market.no_price };

        // Stop loss threshold: 25% drop from fill price
        let stop_price = fill_price * Decimal::from_str("0.75").unwrap();
        ...
        let token_id = match token_id {
            Some(id) => id,
            None => {
                warn!("MintMaker: No token ID for {} side of pair {}, marking StopLoss for auto-redeem", filled_side, pair.id);
