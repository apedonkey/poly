...
import { Filter, Crosshair, Target, Bitcoin, Trophy, ChevronDown } from 'lucide-react'
...
export function OpportunityList() {
  ...
  const filtered = useMemo(() => {
    // First, filter out opportunities that don't meet real-time criteria
    // (these are kept in the list so they can reactivate if price moves back)
    let result = opportunities.filter((o) => o.meets_criteria !== false)
    ...
    switch (filter) {
      case 'sniper':
        ...
        break
      case 'nobias':
        result = result.filter((o) => o.strategy === 'NoBias')
        break
      case 'crypto':
      ...
      case 'sports':
        ...
        break
      default:
        // result already filtered for meets_criteria
        break
    }
  ...
  }, [opportunities, filter, sortBy, sideFilter])
  ...
  const counts = useMemo(() => ({
    ...
    nobias: activeOpportunities.filter((o) => o.strategy === 'NoBias').length,
      ...
      {showPaused && pausedOpportunities.length > 0 && (
        <div className="mb-4 p-3 bg-yellow-500/10 border border-yellow-500/30 rounded-lg">
          ...
          <div className="space-y-2 text-sm">
            {pausedOpportunities.map((opp, index) => (
              <div key={`paused-${opp.market_id}-${index}`} className="flex items-center justify-between text-gray-300 bg-poly-dark/50 rounded px-2 py-1.5">
                <span className="truncate flex-1 mr-2">{opp.question}</span>
                <div className="flex items-center gap-2 text-xs flex-shrink-0">
                  <span className={opp.strategy === 'ResolutionSniper' ? 'text-yellow-400' : 'text-blue-400'}>
                    {opp.strategy === 'ResolutionSniper' ? 'Sniper' : 'NO Bias'}
                  </span>
